...
int main(int argc, char **argv) {
    std::vector<bool> vec(100, true); // 构造大小为 100 的 std::vector<bool>，所有元素初始化为 true
    ...
    {
        vec[20] = false; // 将第 20 个元素设置为 false
    ...
    {
        auto ref = vec[30]; // 获取第 30 个元素的引用
        ASSERT(ref, "Fill in `ref` or `!ref`");
        ref = false; // 通过引用修改第 30 个元素
        ASSERT(!ref, "Fill in `ref` or `!ref`");
        // THINK: WHAT and WHY?
